...
var tomSelect = [ 0, 0, 0, 0, 0, 0, 0 ];

var resonance = 3; //0 is low 3 is max
...
var tomSizes = [ 18, 16, 14, 13, 12, 10, 8 ];
...
case 0:
  if (nToms > 6) {
    dump ('Number of Toms must be 6 or less')
  }
  else {
    ...
    for (i=0; i<nToms; i++) {
      ...
      result += '<tr><td>' + tom[i] +'</td><td>' + Math.floor(octave) +'</td><td>'+ scale[noteLetter] + sharps[note] + '</td><td>'+ fdrum[i].toFixed(0) + '</td><td>'+ ftop[i].toFixed(0) +'</td><td>'+ fbot[i].toFixed(0) +'</td></tr>'
